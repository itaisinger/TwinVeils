...
execute store result score #nearby_ghasts ghast_timer run execute if entity @e[type=ghast,distance=..80]
# If there are 2 or more, stop spawning.
execute if score #nearby_ghasts ghast_timer matches 2.. run return 0
...
spreadplayers ~ ~ 10 50 false @e[type=ghast,tag=just_spawned]
...
scoreboard players set global ghast_cooldown 300